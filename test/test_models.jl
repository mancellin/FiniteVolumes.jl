...
@testset "Anonynous models" begin
    ...
    @testset "(2 var, 1D)" begin
        model = FiniteVolumes.AnonymousModel{SVector{2, Float64}, 1}(u -> SVector(0.5*u[1].^2 + u[2], u[2]))
    ...
    @testset "(2 var, 2D)" begin
        model = FiniteVolumes.AnonymousModel{SVector{2, Float64}, 2}(u -> [SVector(u[1], u[2]), SVector(0.0, 0.0)])
        w = @SVector [1.0, 2.0]
        ...
        h_model = directional_splitting(model)[1]
        @test FiniteVolumes.normal_flux(FiniteVolumes.rotate_model(h_model, [1 0; 0 1]), w) == w

        v_model = directional_splitting(model)[2]
        @test FiniteVolumes.normal_flux(FiniteVolumes.rotate_model(v_model, [1 0; 0 1]), w) == @SVector [0.0, 0.0]
    end
end
